...
namespace Eefa.Bursary.Application.UseCases.Definitions.BankAccount.Commands.Update
{
    public class UpdateBankAccountValidator : AbstractValidator<UpdateBankAccountCommand>
    {
        ...
        public UpdateBankAccountValidator(IBursaryUnitOfWork uow)
        {
            _uow = uow;

            RuleFor(x => x.Id).GreaterThan(0).WithMessage("شناسه شعبه بانک مشخص نشده است");
        ...
        private async Task validateCommand(UpdateBankAccountCommand cmd, ValidationContext<UpdateBankAccountCommand> context, CancellationToken token)
        {
            ...
            var r1 = await _uow.BankAccounts.FirstOrDefaultAsync(x => x.Sheba == cmd.Sheba && x.Id != cmd.Id && x.IsDeleted == false);
            if (r1 != null)
            {
                context.AddFailure("شماره شبای ثبت شده تکراری است");
                return;
            }
            var r2 = await _uow.BankAccounts.FirstOrDefaultAsync(x => x.AccountNumber == cmd.AccountNumber && x.Id != cmd.Id && x.IsDeleted == false);
            if (r2 != null)
            {
