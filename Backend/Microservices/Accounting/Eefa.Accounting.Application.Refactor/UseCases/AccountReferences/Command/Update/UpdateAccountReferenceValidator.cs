...
public class UpdateAccountReferenceValidator : AbstractValidator<UpdateAccountReferenceCommand>
{
    ...
    public UpdateAccountReferenceValidator(IUnitOfWork unitOfWork)
    {
    ...
    {
        return !await _unitOfWork.AccountReferences.ExistsAsync(x => x.Id == id);
    }
    public async Task<bool> beUniqueCode(UpdateAccountReferenceCommand accountReference, CancellationToken cancellationToken)
    {
        return !await _unitOfWork.AccountReferences.ExistsAsync(x => x.Code == accountReference.Code && x.Id != accountReference.Id);
    }
}