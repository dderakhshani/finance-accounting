...
namespace Eefa.Accounting.Application.UseCases.CodeVoucherGroup.Command.Update
{
    public class UpdateCodeVoucherGroupValidator: AbstractValidator<UpdateCodeVoucherGroupCommand>
    {
        ...
        public UpdateCodeVoucherGroupValidator(IRepository repository)
        {
        ...
        public async Task<bool> beUniqueName(UpdateCodeVoucherGroupCommand codeVoucherGroup, CancellationToken cancellationToken)
        {
            return !await _repository.GetQuery<Data.Entities.CodeVoucherGroup>()
                                  .AnyAsync(x => x.UniqueName == codeVoucherGroup.UniqueName && x.Id != codeVoucherGroup.Id);
        }
        public async Task<bool> beUniqueCode(UpdateCodeVoucherGroupCommand codeVoucherGroup, CancellationToken cancellationToken)
        {
            return !await _repository.GetQuery<Data.Entities.CodeVoucherGroup>()
                                 .AnyAsync(x => x.Code == codeVoucherGroup.Code && x.Id != codeVoucherGroup.Id);
        }
    }
}
