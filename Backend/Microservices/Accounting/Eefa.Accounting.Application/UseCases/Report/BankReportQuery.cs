...
namespace Eefa.Accounting.Application.CommandQueries.Report
{
    public class BankReportQuery : Pagination, IMapFrom<BankReportQuery>, IRequest<ServiceResult>, ISearchableRequest, IQuery
    {
    ...
    public class BankReportQueryHandler : IRequestHandler<BankReportQuery, ServiceResult>
    {
        ...
        public BankReportQueryHandler(IAccountingUnitOfWorkProcedures accountingUnitOfWorkProcedures, ICurrentUserAccessor currentUserAccessor, IMapper mapper, IAccountingUnitOfWork context)
        {
        ...
        public async Task<ServiceResult> Handle(BankReportQuery request, CancellationToken cancellationToken)
        {
            ...
            string permissions = permission.GetPermissions<Data.Entities.VouchersDetail>(_context, _currentUserAccessor);
            ...
            if (!string.IsNullOrEmpty(permissions))
                bankinput.UserAccessCondition = permissions;
            var bankresults = await _accountingUnitOfWorkProcedures
                .StpReportBalance6Async(bankinput,
                    cancellationToken: cancellationToken
                );

            bankresults = bankresults.ApplyPermission<StpReportBalance6Result, Data.Entities.VouchersDetail>
            ...
            if (!string.IsNullOrEmpty(permissions))
                Balancebankinput.UserAccessCondition = permissions;
            var Balancebankresults = await _accountingUnitOfWorkProcedures
                .StpReportBalance6Async(Balancebankinput,
                    cancellationToken: cancellationToken
                );
            Balancebankresults = Balancebankresults.ApplyPermission<StpReportBalance6Result, Data.Entities.VouchersDetail>
                                   (_context, _currentUserAccessor, false, false);
            ...
            if (!string.IsNullOrEmpty(permissions))
                Facilitiesinput.UserAccessCondition = permissions;
            var Facilitiesresults = await _accountingUnitOfWorkProcedures
                .StpReportBalance6Async(Facilitiesinput,
                    cancellationToken: cancellationToken
                );
            Facilitiesresults = Facilitiesresults.ApplyPermission<StpReportBalance6Result, Data.Entities.VouchersDetail>
                                   (_context, _currentUserAccessor, false, false);
