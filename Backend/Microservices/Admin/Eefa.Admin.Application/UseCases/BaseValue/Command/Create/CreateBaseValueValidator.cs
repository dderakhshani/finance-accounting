...
namespace Eefa.Admin.Application.UseCases.BaseValue.Command.Create
{
    public class CreateBaseValueValidator: AbstractValidator<CreateBaseValueCommand>
    {
        public CreateBaseValueValidator()
        {
            RuleFor(x => x.BaseValueTypeId)
                .NotEmpty().WithMessage("شناسه نوع اطلاعات پایه نمی تواند خالی باشد.")
                .GreaterThan(0).WithMessage("شناسه نوع اطلاعات پایه وارد شده نمی تواند برابر یا کمتر از0 باشد.");
