...
{
    public class WarehouseLayoutPropertyItemsController : ApiControllerBase
    {
        ICommodityCategoryQueries _Repository;


        public WarehouseLayoutPropertyItemsController(
        ...
        [HttpPost]
        public async Task<IActionResult> GetPropertyItemsByWarehouseLayoutId(int warehouseLayoutId, PaginatedQueryModel paginatedQuery)
        {

            var result = await _Repository.GetPropertyItemsByWarehouseLayoutId(warehouseLayoutId, paginatedQuery);
            return Ok(ServiceResult<PagedList<CommodityCategoryPropertyItemModel>>.Success(result));
