...
import { ArchiveFactorCommand } from '../../../repositories/invoice/commands/invoice/archive-factor-command';
...
export class FactorListComponent extends BaseComponent {
...
toDate: new FormControl(new Date(this.Service.identityService.getActiveYearlastDate())),
 ...
 async initialize() {
  ...
  async navigateToInvoice(Invoice: invoice) {
    await this.router.navigateByUrl(`purchase/invoice-operations/addFactor?id=${Invoice.id}&displayPage=edit`)

  }
  async navigateToCopyReceipt(Invoice: invoice) {
    await this.router.navigateByUrl(`purchase/invoice-operations/addFactor?id=${Invoice.id}&displayPage=copy`)
  ...
  async archive(Invoices: invoice) {
    ...
    dialogRef.afterClosed().subscribe(async result => {
      if (result == true) {
        let response = await this._mediator.send(new ArchiveFactorCommand(Invoices.id));
