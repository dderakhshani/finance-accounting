...
@Component({
  selector: 'app-archive-factor-list',
  templateUrl: './archive-factor-list.component.html',
  styleUrls: ['./archive-factor-list.component.scss'],
  providers: [DatePipe]
})
export class ArchiveFactorListComponent extends BaseComponent {
  ...
  InvoiceAllStatusUnicCode: string = 'ArchiveFactor';
...
toDate: new FormControl(new Date(this.Service.identityService.getActiveYearlastDate())),
  ...
  async get() {
    ...
    let id = this.ApiCallService.InvoiceStatus.find(a => a.uniqueName == this.Service.ArchiveFactor)?.id
