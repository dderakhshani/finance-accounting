...
@Component({
  selector: 'app-add-start-document-receipt',
  templateUrl: './add-start-document-receipt.component.html',
  styleUrls: ['./add-start-document-receipt.component.scss']
})
export class AddStartDocumentReceiptComponent extends BaseComponent {
  ...
  async add() {
    ...
    if (this.form.controls['codeVoucherGroupId'].value == undefined) {
      ...
      this.Service.showHttpFailMessage('نوع سند را بدرستی انتخاب نمایید');
      return;
    ...
    this.form.controls.documentStauseBaseValue.setValue(DocumentState.invoiceAmountStart);

    await this._mediator.send(<CreateStartDocumentCommand>this.request).then(response => {
  ...
  async navigateToReceipt(Receipt: Receipt) {
    await this.router.navigateByUrl(`inventory/rialsReceiptDetails?id=${Receipt.id}`)
  }
