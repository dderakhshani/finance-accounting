...
@Component({
  selector: 'app-Rials-receipt-list',
  templateUrl: './Rials-receipt-list.component.html',
  styleUrls: ['./Rials-receipt-list.component.scss']
})
export class RialsReceiptListComponent extends BaseComponent {
  ...
  ReceiptAllStatusUnicCode: string = 'InventoryReceiptCost';
  ...
  async resolve() {
    ...
    let colCurrencyPrice = new TableColumn(
      'currencyPrice',
      'قیمت ارزی',
      TableColumnDataType.Template,
      '5%',
      true,
      new TableColumnFilter('currencyPrice', TableColumnFilterTypes.Number)
    )
   ...
   let colTotalItemPrice= new TableColumn(
      'totalItemPrice',
      'مبلغ کل ',
      TableColumnDataType.Money,
      '20%',
      true,
      new TableColumnFilter('totalItemPrice', TableColumnFilterTypes.Money)
    )
    ...
    colCurrencyPrice.template = this.txtCurrencyPrice;
    ...
    if (this.isImportPurchase == 'false') {
      ...
      columns.push(colTotalItemPrice)
  ...
  async navigateToRialReceipt(Receipt: Receipt) {
    await this.router.navigateByUrl(`inventory/rialsReceiptDetails?documentId=${Receipt.documentId}&isImportPurchase=${this.isImportPurchase}`)
  }
