...
@Component({
  selector: 'app-commodity-receipt-reports-rial',
  templateUrl: './commodity-receipt-reports-rial.component.html',
  styleUrls: ['../commodity-receipt-reports.component.scss']
})

export class CommodityReceiptReportsRialComponent extends BaseSetting {
  ...
  CalculateSum(response:any[],isSelectedRows:boolean = false) {
    ...
    if (isSelectedRows) {
      ...
      this.prefix_TotalItemPrice = this.data.reduce((sum, current) => sum + Number(current.prefix_TotalItemPrice), 0);
      this.current_Enter_TotalItemPrice = this.data.reduce((sum, current) => sum + Number(current.current_Enter_TotalItemPrice), 0);
      this.current_Exit_TotalItemPrice = this.data.reduce((sum, current) => sum + Number(current.current_Exit_TotalItemPrice), 0);
      ...
      this.prefix_Quantity = this.data.reduce((sum, current) => sum + Number(current.prefix_Quantity), 0);
      ...
      this.postfix_Quantity = this.data.reduce((sum, current) => sum + Number(current.postfix_Quantity), 0);
  ...
  async updateAvgPrice(item: any) {


      let UpdateAvgPrice = new UpdateAvgPriceAfterChangeBuyPriceCommand();
      UpdateAvgPrice.documentItemId = item?.documentItemId;
