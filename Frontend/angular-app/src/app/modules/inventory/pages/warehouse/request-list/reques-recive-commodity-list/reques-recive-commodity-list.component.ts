...
export class RequesReciveCommodityListComponent extends BaseComponent {
  ...
  async navigateToReceipt(Receipt: Receipt, GroupCode :number) {
    await this.router.navigateByUrl(`inventory/leavingMaterialWarehouse?RequestId=${Receipt.id}&GroupCode=${GroupCode}`)
  }
