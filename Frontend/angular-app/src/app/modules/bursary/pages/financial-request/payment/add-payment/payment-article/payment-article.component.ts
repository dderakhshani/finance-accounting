...
import { AccountHeads, AccountReferencesGroupEnums, AccountTypes, CodeVouchers, CurrencyType, Documents, FinantialReferences } from 'src/app/modules/bursary/entities/enums';
...
import { AddReceiveChequeComponent } from '../../../receive/cheque/add-receive-cheque.component';
...
import { FinancialPaymentPartial } from 'src/app/modules/bursary/entities/financial-request-partial';
...
@Component({
  selector: 'app-payment-article',
  templateUrl: './payment-article.component.html',
  styleUrls: ['./payment-article.component.scss']
})

export class PaymentArticleComponent extends BaseComponent {
  ...
  constructor(private router: Router,private route: ActivatedRoute,private _mediator: Mediator, public dialog: MatDialog ,@Optional() public dialogRef: MatDialogRef<PaymentArticleComponent>, @Inject(MAT_DIALOG_DATA) public value: any) {
    super(route, router);

    this.form = <FormGroup>this.createForm(new CreatePaymentCommand(), true);
         const chequeSheet = <FormGroup>this.createForm(new CreateChequeSheetCommand(), true);
         this.form.controls['chequeSheet'] = chequeSheet;
  }
  ...
  async ngOnInit() {
    ...
    if (this.value.data != null && this.value.data != "null") {
      let data = JSON.parse(this.value.data);

      me.documentTypes= data.documentTypes;
      me.currencyTypes = data.currencyTypes;
      me.referenceTypes = data.referenceTypes;
      me.chequeTypes = data.chequeTypes;
      me.finantialReferenceTypes = data.finantialReferenceTypes;
      me.chequeSheets = data.chequeSheets;
      me.banks = data.banks;
    }
  ...
  createDefaultRequest() {
    ...
    if (entity || this.getQueryParam("id")) {
      ...
      if (!entity) entity =   this.get(this.getQueryParam("id"));
      this.request = new UpdatePaymentCommand().mapFrom(<FinancialRequest>entity);
    } else {
...
var me = this;
    ...
    let accountReferenceGroupId = accountType === AccountTypes.Credit ? me.form.controls['creditAccountReferenceGroupId']?.value : me.form.controls['debitAccountReferenceGroupId']?.value
    let isAccountReferenceGroupIdNumeric = typeof accountReferenceGroupId === 'number'
    let accountHeadId = accountType === AccountTypes.Credit ? me.form.controls['creditAccountHeadId']?.value : me.form.controls['debitAccountHeadId']?.value;
    let isAccountHeadIdNumeric = typeof accountHeadId === 'number'
    let accountReferenceId = accountType === AccountTypes.Credit ? me.form.controls['creditAccountReferenceId']?.value : me.form.controls['debitAccountReferenceId']?.value;
    ...
    if (!isAccountReferenceGroupIdNumeric && isAccountHeadIdNumeric && !isAccountReferenceIdNumeric) {
      filter.push({
        propertyName: 'id',
        comparison: 'in',
        values: accountType === AccountTypes.Credit ? [...(me.creditAccountHeads.find(x => x.id === accountHeadId)?.accountReferenceGroupsIds as Number[])] : [...(me.debitAccountHeads.find(x => x.id === accountHeadId)?.accountReferenceGroupsIds as Number[])],
        nextOperand: 'and'
      })

      if (accountType === AccountTypes.Credit) {
        me.form.controls['creditAccountReferenceId'].disable({ emitEvent: false })
        me.form.controls['creditAccountReferenceId'].setValue(null, { emitEvent: false })
      } else {
        me.form.controls['debitAccountReferenceId'].disable({ emitEvent: false })
        me.form.controls['debitAccountReferenceId'].setValue(null, { emitEvent: false })
      }
    } else {



      accountType === AccountTypes.Credit ? me.form.controls['creditAccountReferenceId'].enable({ emitEvent: false }) : me.form.controls['debitAccountReferenceId'].enable({ emitEvent: false })
    }
    ...
    if (!isAccountReferenceGroupIdNumeric && isAccountReferenceIdNumeric && !isAccountHeadIdNumeric) {
      filter.push({
        propertyName: 'id',
        comparison: 'in',
        values: [...(me.creditAccountReferences.find(x => x.id === accountReferenceId)?.accountReferencesGroupsIdList as Number[])],
        nextOperand: 'and'
      })

      if (accountType === AccountTypes.Credit) {
        me.form.controls['creditAccountHeadId'].disable({ emitEvent: false })
        me.form.controls['creditAccountHeadId'].setValue(null, { emitEvent: false })
      } else {
        me.form.controls['debitAccountHeadId'].disable({ emitEvent: false })
        me.form.controls['debitAccountHeadId'].setValue(null, { emitEvent: false })
      }
    } else {
      accountType === AccountTypes.Credit ? me.form.controls['creditAccountHeadId'].enable({ emitEvent: false }) : me.form.controls['debitAccountHeadId'].enable({ emitEvent: false })
    }

    if (isAccountReferenceGroupIdNumeric && (me.pageMode === PageModes.Update || ((isAccountHeadIdNumeric && !isAccountReferenceIdNumeric) || (!isAccountHeadIdNumeric && isAccountReferenceIdNumeric)))) {
      filter.push({
        propertyName: 'id',
        comparison: 'equal',
        values: [accountReferenceGroupId],
    ...
    me.isLoading = true;
    await me._mediator.send(new GetBursaryAccountReferenceGroupsQuery(0, 25, filter)).then((res) => {
    //  accountType === AccountTypes.Credit ? me.creditAccountReferenceGroups = res.data : me.debitAccountReferenceGroups = res.data // خطا میداد کامنت کردم باید درست شود برنامه کار نمیکند
    });
    me.isLoading = false;
