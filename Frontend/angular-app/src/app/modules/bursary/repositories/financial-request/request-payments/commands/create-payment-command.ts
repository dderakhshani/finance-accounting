...
export class CreatePaymentCommand extends IRequest<CreatePaymentCommand,FinancialPaymentPartial> {
  ...
  mapFrom(entity: FinancialPaymentPartial): CreatePaymentCommand {
  ...
  mapTo(): FinancialPaymentPartial {
    throw new ApplicationError(
      CreatePaymentCommand.name,
      this.mapTo.name,
      "Not Implemented Yet"
    );
  }
...
@MediatorHandler(CreatePaymentCommandHandler.name)
export class CreatePaymentCommandHandler
  implements IRequestHandler<CreatePaymentCommand, FinancialPaymentPartial>
{
  constructor(
  ...
  async Handle(request: CreatePaymentCommand): Promise<FinancialPaymentPartial> {
    let httpRequest: HttpRequest<CreatePaymentCommand> = new HttpRequest<CreatePaymentCommand>(request.url, request);

    return await this._httpService
      .Post<CreatePaymentCommand, ServiceResult<FinancialPaymentPartial>>(httpRequest)
      .toPromise()
      .then((response) => {
